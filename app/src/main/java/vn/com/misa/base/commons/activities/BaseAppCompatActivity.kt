package vn.com.misa.base.commons.activities
...
import vn.com.base.libraries.views.textviews.ExtTextView
import vn.com.misa.base.BuildConfig
...
import vn.com.misa.base.utilities.RxUtility
import javax.inject.Inject

abstract class BaseAppCompatActivity<V : IBaseActivityMvpView, P : IBaseActivityMvpPresenter<V>> : AppCompatActivity(),
    IBaseActivityMvpView {
    companion object {
...
//    override fun <T> bindToLifecycle(): LifecycleTransformer<T> = RxLifecycleAndroid.bindActivity(lifecycleSubject)
     ...
     * register bus
     */
    override fun <T : IEvent> registerRxBus(
        clazz: Class<T>,
        consumer: (T) -> Unit
    ) {
        RxUtility.registerRxBus(rxBus, mCompositeDisposable, clazz, consumer)
    }
     ...
     * @param isLoading
     */
    override fun showLoadingInView(isLoading: Boolean) {
    ...
    override fun showDialogLoading(isLoading: Boolean) = if (isLoading) {
        showProgressDialog()
    } else {
        hideProgressDialog()
    }
    ...
    override fun showMessage(message: String?, ok: String, consumer: () -> Unit) {
        DialogUtility.messageBox(
            this,
            getString(R.string.app_name),
            message,
            ok,
            DialogInterface.OnClickListener { _, _ -> consumer() })
    }
    ...
    override fun showMessage(
        ...
        DialogUtility.messageBox(this,
            getString(R.string.app_name),
            message,
            ok,
            cancel,
            DialogInterface.OnClickListener { _, _ -> okConsumer() },
            DialogInterface.OnClickListener { _, _ -> cancelConsumer() })
    }

    override fun showLoadingInPagingView(isLoading: Boolean) {}
    ...
    fun registerNetworkEventListener() {
        registerRxBus(NetworkEvent::class.java) {
            when (it.errorCode) {
                ErrorCodes.UNKNOWN -> {
                }
...
//        CollectionUtility.with(fragments)
//            .doIfEmpty { BackUtility.onClickExit(this, getString(R.string.double_tap_to_exit)) }
...
//    }
     ...
     * network listener
     */
    class ConnectionStateMonitor(var context: Context, var rxBus: RxBus<IEvent>) :
